...
if __name__ == "__main__":
    """
    This script aggregates in a single dataframe all the attributions from COVID patients with blood and lung in the validation data.
    """
    ...
    for i_f, f in enumerate(os.listdir(ig_path)):
        if (class_type in f) and (f"_{target_id}.csv" in f) and ("indexed" in f) and ("all" in f):
            print(f)
            if grep_for_word(ig_path + f, word="COVID-19") or (grep_for_word(ig_path + f, word="healthy")):
                print("grep found")
                df = pd.read_csv(ig_path + f)
                print("loaded")
                
                # Only tissue lung or blood and COVID19 patients
                #df = df.loc[(df["disease"]=="COVID-19") & (df["tissue"].isin(["blood", "lung"]))]

                gene_cols = [str(c) for c in df.columns if c not in ['disease','tissue','cell_type','study::::sample','cell_index']]
    ...
    ann_covid.write_h5ad("/homefs/home/debroue1/projects/cellm/data/pascient/attributions/all_indexed_attributions_binary_covid.h5ad", compression="gzip")
    print("Saved to disk")